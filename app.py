...
GEMINI_DEFAULT_WAIT_FOR_AVAILABLE = 15.0
...
def build_engine_config(config_map: Dict[str, str]) -> EngineConfig:
    ...
    return EngineConfig(
        ...
        gemini_wait_for_available=as_float(
            config_map.get("gemini_wait_for_available"),
            default=GEMINI_DEFAULT_WAIT_FOR_AVAILABLE,
        ),
    ...
    if config.use_gemini:
        if not config.gemini_keys:
            ...
            while True:
                ...
                if cooldown_waits and wait_budget > 0:
                    wait_for = min(max(0.0, min(cooldown_waits)), wait_budget)
                    if wait_for > 0:
                        logger.info(
                        ...
                        time.sleep(wait_for)
                        wait_budget -= wait_for
                        continue

                if attempted and had_rate_limit and wait_budget > 0:
                    earliest_available = [
                        max(0.0, GEMINI_KEY_COOLDOWNS.get(api_key, 0.0) - time.time())
                        for api_key in config.gemini_keys
                    ]
                    earliest_available = [value for value in earliest_available if value > 0]
                    if earliest_available:
                        wait_for = min(min(earliest_available), wait_budget)
                        if wait_for > 0:
                            logger.info(
                                "Gemini keys cooling down after rate limits; waiting %.2f seconds",
                                wait_for,
                            )
                            time.sleep(wait_for)
                            wait_budget -= wait_for
...
def paraphrase_element(element, config: EngineConfig) -> None:
    ...
    def flush_segment() -> None:
        ...
        placeholder_entries: List[Dict[str, str]] = []
        ...
        for entry in placeholder_entries:
            ...
            pos = combined.find(placeholder, search_from)
            if pos == -1:
                continue
            if pos > 0:
                entry["before_char"] = combined[pos - 1]
            after_index = pos + len(placeholder)
            if after_index < len(combined):
                entry["after_char"] = combined[after_index]
            search_from = pos + len(placeholder)
        ...
        for entry in placeholder_entries:
            placeholder = entry["placeholder"]
            before_char = entry.get("before_char", "")
            after_char = entry.get("after_char", "")
            inline_text = entry.get("text", "")
            trimmed_inline = inline_text.strip()

            if before_char and not before_char.isspace():
                pattern = re.compile(rf"(?<=\S)\s+{re.escape(placeholder)}")
                result_core = pattern.sub(placeholder, result_core)

            if after_char and not after_char.isspace():
                pattern = re.compile(rf"{re.escape(placeholder)}\s+(?=\S)")
                result_core = pattern.sub(placeholder, result_core)

            if (not before_char or before_char.isspace()) and trimmed_inline:
                first_char = trimmed_inline[0]
                pattern = re.compile(
                    rf"(?:(?<=^)|(?<=\s)){re.escape(first_char)}\s+{re.escape(placeholder)}"
                )
                result_core = pattern.sub(placeholder, result_core, count=1)

            if (not after_char or after_char.isspace()) and len(trimmed_inline) > 1:
                remainder = trimmed_inline[1:]
                pattern = re.compile(
                    rf"{re.escape(placeholder)}\s*{re.escape(remainder)}(?=(?:\s|$|[.,;:!?]))"
                )
                result_core = pattern.sub(placeholder, result_core, count=1)

        for entry in placeholder_entries:
            result_core = result_core.replace(entry["placeholder"], entry["html"])
